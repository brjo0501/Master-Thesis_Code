...
function speedChange(ui,id,newVal)
    ...
    if v then
        ...
        if v~=c['velocity'] then
            simBWF.markUndoPoint()
            d['speed'] = v*1000
            c['velocity'] = d['speed']/1000
            writeInfo(c)
            writeCustomInfo(d)
        end
    end
    simUI.setEditValue(ui,10,simBWF.format("%.0f",d['speed']),true)
end
...
function createDlg()
    ...
    <tab title="More" layout="form">
        ...
        simUI.setEditValue(ui,10,simBWF.format("%.0f",customConfig['speed']),true)
...
function sysCall_init()
    ...
    writeCustomInfo({speed = data['velocity']*1000})
