...
function sysCall_init()
    ...
    buffer1_2 = RingBuffer:new(25)
    writeBuffer(buffer1,'buffer1_2')

    buffer2_2 = RingBuffer:new(25)
    writeBuffer(buffer2,'buffer2_2')
...
function sysCall_actuation()
    ...
    -- trigger2 = sim.unpackTable(sim.readCustomDataBlock(rob2,'partTrigger'))
    -- if trigger2['trigger'] then
    --     sim.writeCustomDataBlock(rob2,'partTrigger',sim.packTable({trigger = false}))
    --     oldIndex = buffer1:pop()[1]
    --     speeds = subTable(speeds,oldIndex)
    ...
    if trigger2['trigger'] then
        ...
        for i=1, (#speeds) do
            speedData[i] = speeds[i]
        end

        for i=1, newIndex do
            speeds[i] = nil
        end
        buffer2:push({#speedData[newIndex],average(speedData),var(speedData,average(speedData))})
        print(buffer2)
    end
