...
customData = sim.readCustomDataBlock(model,'customData')

--   inter = sim.getObject('/Interventions')
--   cam = sim.getObject('/camera_1/camera')
--   index = 1

--   interData = sim.readCustomDataBlock(inter,'customData')

--   camData = sim.unpackTable(sim.readCustomDataBlock(cam,'customData'))
--   enabledInter = sim.unpackTable(interData)
...
--   --print(enabledDetection['enabledDetection'] and camDetect)
  ...
  if enabledDetection then --and not enabledInter['interDetection'] then 
