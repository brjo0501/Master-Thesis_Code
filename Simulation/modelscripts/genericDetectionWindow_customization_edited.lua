...
function setDlgItemContent()
    if ui then
        ...
        local customConfig = readCustomInfo()
        local sel=simBWF.getSelectedEditWidget(ui)
        simUI.setCheckboxValue(ui,10,simBWF.getCheckboxValFromBool(customConfig['enabledDetection']~=false),true)
        --print(customConfig['enabledDetection'])
...
function enable_callback(ui,id,newVal)
    local c=readCustomInfo()
    if newVal == 2 then
        c['enabledDetection'] = true
    else
        c['enabledDetection'] = false
    end
    ...
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            <tab title="General properties" layout="form">
                ...
                <label text="Enabled"/>
                <checkbox text="" on-change="enable_callback" id="10" />
...
function sysCall_init()
    ...
    customTable = {enabledDetection = true}
