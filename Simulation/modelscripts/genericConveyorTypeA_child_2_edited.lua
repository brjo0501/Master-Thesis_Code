...
function sysCall_init()
    ...
    sim.writeCustomDataBlock(model,'partTrigger-Cam2',sim.packTable({trigger = false}))

    index2 = 0
    newIndex2 = 0
    oldIndex2 = 0
    speeds2 = RingBuffer:new(1500)
...
function sysCall_actuation()
    ...
    trigger1 = sim.unpackTable(sim.readCustomDataBlock(model,'partTrigger-Cam2'))
    if trigger1['trigger'] then
        sim.writeCustomDataBlock(model,'partTrigger-Cam2',sim.packTable({trigger = false}))
        newIndex2 = {index2}
        bufferIndex:push(newIndex2)
    end
    
    index2 = index2 + 1

    speeds2:push({speed_noise})

    if oldIndex2 ~= bufferIndex:peek()[1] and bufferIndex:peek()[1] then
        step = step + 1
        peekLength = 1976.25/(base_speed*0.05)
        if step  == peekLength or (step == 161 and oldIndex2 ~= 0)then
            newIndex2 = bufferIndex:pop()[1]
            speedData = {}
            
            speedData = speeds2:peekBuffer(peekLength)

            --sim.pauseSimulation()
            for i=1, (newIndex2-oldIndex2) do
                speeds2:pop()
            end
    ...
    -- print(#speeds2)
    -- print(1976.25/(base_speed*0.05)+bufferIndex:peek()[1])
    
    -- if  #speeds2 == 1976.25/(base_speed*0.05)+bufferIndex:peek()[1] and bufferIndex:peek()[1] > 0 then -- distance up to tracking window minus part width
    --     newIndex2 = bufferIndex:pop()[1]
    --     speedData = {}
    --     speedsTemp = {}

    --     for i=newIndex2, (#speeds2) do
    --         speedData[i-newIndex2+1] = speeds2[i]
    --         speedsTemp[i] = speeds2[i]
    --     end
