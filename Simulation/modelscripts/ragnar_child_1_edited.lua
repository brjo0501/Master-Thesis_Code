...
function sysCall_init()
    ...
    local data = sim.unpackTable(sim.readCustomDataBlock(model,'customData'))
    base_vel = data['maxVel']
    base_gripper = data['gripperSupply']
    maxVel_noise = 0
    gripper_noise = 0
    phase_shift1 = math.random()*math.pi*2
    phase_shift2 = math.random()*math.pi*2

    events = sim.getObject('/Events')

    buffer1 = RingBuffer:new(7)
    buffer2 = RingBuffer:new(7)
    buffer3 = RingBuffer:new(7)
    buffer4 = RingBuffer:new(7)
...
function sysCall_actuation()
    ...
    joint3=sim.getObject('./Ragnar_A1DrivingJoint3')
    joint4=sim.getObject('./Ragnar_A1DrivingJoint4')

    data['jointVelo1'] = sim.getJointVelocity(joint1)
    ...
    if data['gripperSupply'] > 0 then
        gripper_noise = base_gripper + noise2
        if gripper_noise <= 100 and gripper_noise >= 99.5 then
            data['gripperSupply'] = gripper_noise
        end
    end

    if data['gripperVacuum'] > 0 then
        if (gripper_noise <= 100 and gripper_noise >= 99.5) and (noise3 < 0.5 and noise3 > -0.5) then
            data['gripperVacuum'] = gripper_noise - 10 + noise3
        end
    end
    ...
    if trackingWindowHandle>=0 then
        ...
        if trackedTargets then
            ...
            for key,value in pairs(trackedTargets) do
                local nameIsSame=false
                for i=1,#allLocationNames,1 do
                   if allLocationNames[i]==value['partName'] then --true, for all pick ups
                        nameIsSame=true
                        break
                    end
                end
...
ragnar_attachPart=function(part)
    local partHandle=part['partHandle']
    local customData = sim.unpackTable(sim.readCustomDataBlock(model,'customData'))
    --print(customData)
    if customData['gripperSupply'] > 50 then
        customData['gripperVacuum'] = customData['gripperSupply']-10
...
RobPick = function(partData,attachPart,theStackingShift,approachHeight,blend,nulling,dwTime)
    ...
    local eventInput = {['pick 1']=sim.getSimulationTime(),['assembly'] = assemblyCounter}
    --print('rob 1 - '..sim.getSimulationTime())
...
--partOr[3]/angularGain
        ...
        --print(curPVA,MaxVAJ,selVec,partPV)
        res,nextPVA=sim.rmlStep(rmlObj,dt)
        sim.rmlRemove(rmlObj)
...
RobPlace = function(TrackPart,detachPart,approachHeight,blend,nulling,dwTime,attachToTrackingLocation)
    ...
    local event = sim.readCustomDataBlock(events,'customData')
    local eventList = sim.unpackTable(event)
    local eventInput = {['place 1']=sim.getSimulationTime(),['assembly'] = assemblyCounter}
    local newEntry = #(eventList) + 1
    eventList[newEntry] = eventInput

    buffer1 = readBuffer('buffer1')
...
function coroutineMain()
    ...
    sim.setJointTargetVelocity(fkDrivingJoints[1], 0.0, {0.0, 0.0})
